...
/**
 * Pointer-driven scrubber adapted from smoothui.dev/docs/components/scrubber.
 * Behaviour preserved verbatim; styling uses shadcn semantic tokens
 * (`bg-muted`, `bg-foreground`, `text-foreground`) so it picks up the
 * project's tokens.css colour scheme automatically.
 */
function Scrubber({
  label = "Value",
  ...
  useEffect(() => {
    ...
    // Initial sync read of the media-query state; the subscription below
    // keeps it in sync. This is the canonical "subscribe to an external
    // source" pattern — not a derived-state effect.
