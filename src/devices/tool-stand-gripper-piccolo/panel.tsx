...
export function Panel() {
  const s = useDeviceState<ToolStandGripperPiccoloState>(
  ...
  return (
    <div className="flex flex-col gap-3 px-3 pt-1 pb-3">
      <DataSection title="Gripper piccolo">
...
function ValveGrid({
  ...
  ventose: ToolStandGripperPiccoloState['ventose']
}) {
  function toggleValvola(index: number) {
    const store = useMachineStore.getState()
    const prev = store.devices['tool-stand-gripper-piccolo'] as
      | ToolStandGripperPiccoloState
      | undefined
    if (!prev) return
    const next = prev.ventose.map((v, i) =>
      i === index ? { ...v, attiva: !v.attiva } : v,
    )
    store.setDevice('tool-stand-gripper-piccolo', { ...prev, ventose: next })
  }
